...
public class Network_Manager
{
    ...
    public void Start_Network_Service()
    {
        ...
        dbConnection = database_Manager.Start_DB_Connection();
    }

    public void Close_DB_Connection()
    {
        database_Manager.Close_DB_Connection(dbConnection);
    }
    ...
    private void Login(string nick, string password, Client client)
    {
        ...
        List<string> returnList = database_Manager.OptimizedStringSelect(dbConnection, query, "id_user");

        if (returnList.Count == 0)
        {
            IncorrectLogIn(client);
        }
        else
        {
            CorrectLogIn(client);
    ...
    private void Register(string nick, string password, string raza, Client client)
    {
        //Verificariamos los datos de login
        string query = "SELECT * FROM users WHERE nick = '" + nick + "' AND pass = '" + password + "';";
        ...
        if (returnList.Count == 0)
        {
            query = "Insert Into users (nick, pass, raza_id) Values ('" + nick + "', '" + password + "', " + raza + ");";

            database_Manager.OptimizedInsert(dbConnection, query);

            CorrectRegister(client);
        }
        else
        {
            IncorrectRegister(client);
        }
    }
    ...
    private void CorrectLogIn(Client client)
    {
        try
        {
            //Instanciamos el writer para enviar el mensaje de ping
    ...
    private void IncorrectLogIn(Client client)
    {
        try
        {
            //Instanciamos el writer para enviar el mensaje de ping
    ...
    private void CorrectRegister(Client client)
    {
        try
        {
            //Instanciamos el writer para enviar el mensaje de ping
    ...
    private void IncorrectRegister(Client client)
    {
        try
        {
            //Instanciamos el writer para enviar el mensaje de ping
            ...
            //Enviamos el ping
            writer.WriteLine("incorrectRegister");
